...
using Yiwan.Helpers.Http.Enum;
using Yiwan.Helpers.Http.Static;

namespace Yiwan.Helpers.Http.Base
{
    /// <summary>
    ...
    internal class HttphelperBase
    {
        ...
        internal HttpResult GetHtml(HttpItem item)
        {
            ...
            {
                //配置参数时出错
                return new HttpResult() { Cookie = string.Empty, Html = ex.Message, StatusDescription = "配置参数时出错：" + ex.Message };
            }
            try
            {
                //请求数据
        ...
        internal HttpResult FastRequest(HttpItem item)
        {
            ...
            {
                //配置参数时出错
                return new HttpResult() { Cookie = response.Headers["set-cookie"] ?? string.Empty, Html = ex.Message, StatusDescription = "配置参数时出错：" + ex.Message };
            }
            try
            {
                //请求数据
                ...
                {
                    //成功 不做处理只回成功状态
                    HttpResult rsHttp = new HttpResult() { Cookie = response.Headers["set-cookie"] ?? string.Empty, StatusCode = response.StatusCode, StatusDescription = response.StatusDescription };
                    rsHttp.SetHeader(response.Headers);
                    return rsHttp;
                }
            }
            catch (WebException ex)
        ...
        private void GetData(HttpItem item, HttpResult result)
        {
            ...
            //获取Headers
            result.SetHeader(response.Headers);
            //获取CookieCollection
            if (response.Cookies != null) result.SetCookieCollection(response.Cookies);
            //获取set-cookie
            ...
            if (item.IsUpdateCookie)
            {
                ...
                item.SetCookieCollection(result.CookieCollection);
            }
            #endregion
        ...
        private void SetRequest(HttpItem item)
        {
            ...
            if (item.IfModifiedSince != null) request.IfModifiedSince = Convert.ToDateTime(item.IfModifiedSince, CultureInfo.InvariantCulture);
            //Accept
        ...
        private void SetPostData(HttpItem item)
        {
            ...
            if (!request.Method.Trim().ToLower(CultureInfo.InvariantCulture).Contains("get"))
            {
                ...
                byte[] buffer = null;
                //写入Byte类型
                if (item.PostDataType == PostDataType.Byte && item.GetPostdataByte() != null && item.GetPostdataByte().Length > 0)
                {
                    //验证在得到结果时是否有传入数据
                    buffer = item.GetPostdataByte();
                }//写入文件
        ...
        private void SetProxy(HttpItem item)
        {
            ...
            if (!string.IsNullOrWhiteSpace(item.ProxyIp) && !isIeProxy)
            {
                ...
                if (item.ProxyIp.Contains(":"))
                {
                    ...
                    WebProxy myProxy = new WebProxy(plist[0].Trim(), Convert.ToInt32(plist[1].Trim(), CultureInfo.InvariantCulture))
                    {
                        //建议连接
